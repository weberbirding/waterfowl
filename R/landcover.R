...
landcover_percent <- function(data, raster.source = "FedData",
  ...
  if (is(raster.source, "character")){
    message("Downloading NLCD 2019 raster for your extent...")
...
#' @import move
#' @importFrom methods is
landcover_points <- function(points, raster.source = "FedData", add = FALSE){
  if (is.list(points)){
    results <- plyr::llply(points, landcover_points, raster.source = raster.source,
                           add = FALSE, .progress = "text")
    return(results)
  }
  ...
  if (add && is(points, "SpatVector")) {
    landcover <- terra::extract(spat, vec, ID = FALSE, bind = TRUE)
  } else {
    landcover <- terra::extract(spat, vec, ID = FALSE)
  }
  if (!add) {
    return(landcover)
  } else {
    if (is(points, "Move") || is(points, "MoveStack")) {
      points@data$landcover <- landcover[ , 1]
